...
class DeepBKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if self.emb_type == "qid":
            ...
            self.forgetting = False
            self.difficulty = False
        elif self.emb_type == "bayesian_v5":
            self.augmentation = False
            self.bayesian = True
            self.forgetting = False
            self.difficulty = False
        ...
        if self.augmentation or self.bayesian:
            ...
            if self.bayesian:
                self.tanh = nn.Tanh()
                self.sigmoid = nn.Sigmoid()
                self.bayesian_linear1 = nn.Sequential(
                    nn.Linear(d_model + embed_l,
                            embed_l), nn.ReLU(), nn.Dropout(self.dropout)
                )
                self.mastery = nn.Embedding(self.n_question + 1, embed_l)
                if emb_type != "bayesian":
                    if emb_type not in ["bayesian_v4", "bayesian_v5"]:
                        self.bayesian_linear1 = nn.Linear(embed_l + d_model, embed_l)
                        if emb_type.find("bayesian_v2") != -1 or emb_type.find("bayesian_v3") != -1:
                        ...
                        self.mastery = nn.Embedding(self.n_question + 1, embed_l + d_model)
                        self.bayesian_linear2 = nn.Linear(embed_l + d_model, embed_l)
                        self.bayesian_linear3 = nn.Linear(embed_l + d_model, embed_l)
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        # 先bayesian再进行归一化
        ...
        elif self.bayesian and not self.augmentation and emb_type == "bayesian_v4":
            kc_slipping = self.slipping(q_data)
            kc_slipping = self.Sigmoid(kc_slipping)
            kc_guess = self.guess(q_data)
            kc_guess = self.Sigmoid(kc_guess)
            ...
            output = bayesian_final_input * (1 - kc_slipping) + (d_mastery - bayesian_final_input) * kc_guess
        # 先bayesian再进行归一化
        elif self.bayesian and not self.augmentation and emb_type == "bayesian_v5":
            kc_slipping = self.slipping(q_data)
            kc_slipping = self.Sigmoid(kc_slipping)
            kc_guess = self.guess(q_data)
            kc_guess = self.Sigmoid(kc_guess)
        ...
        elif self.bayesian and self.augmentation and emb_type == "augmentation_bayesian":
            # print(f"using augmentation and bayesian")
            tmp_output = d_output.reshape(2, batch_size, seqlen, -1)
            ...
            tmp_q_embed_data = final_q_embed_data.reshape(2, batch_size, seqlen, -1)
            bayesian_input = torch.cat([tmp_output[0], tmp_q_embed_data[0]], dim=-1)
            bayesian_input = self.bayesian_linear1(bayesian_input)
            new_ouput = bayesian_input * (1 - kc_slipping) + (d_mastery - bayesian_input) * kc_guess
            output = torch.cat([new_ouput, tmp_output[1]], dim=0)
        # 先归一化再进行bayesian
        elif self.bayesian and self.augmentation and emb_type == "augmentation_bayesian_v2":
            # print(f"using augmentation and bayesian")
            tmp_output = d_output.reshape(2, batch_size, seqlen, -1)
            new_qdata = q_data.reshape(2, batch_size, -1)
            kc_slipping = self.slipping(new_qdata[0])
            ...
            bayesian_final_input = (bayesian_input_tmp + 1)/2 * bayesian_gain
            bayesian_final_input = self.bayesian_linear4(bayesian_final_input)
            new_ouput = bayesian_final_input * (1 - kc_slipping) + (d_mastery - bayesian_final_input) * kc_guess
            output = torch.cat([new_ouput, tmp_output[1]], dim=0)
        elif self.bayesian and self.augmentation and emb_type == "augmentation_bayesian_v3":
            # print(f"using augmentation and bayesian")
            tmp_output = d_output.reshape(2, batch_size, seqlen, -1)
            new_qdata = q_data.reshape(2, batch_size, -1)
            kc_slipping = self.slipping(new_qdata[0])
            ...
            output = (output_tmp + 1)/2 * output_gain
            output = self.bayesian_linear4(output)
            output = torch.cat([output, tmp_output[1]], dim=0)

        if emb_type == "qid" or emb_type.find("forgetting") != -1:
