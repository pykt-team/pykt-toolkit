...
from .peiyou_emb_fusion_add import QuestionEncoder, KCRouteEncoder, GateEarlyFusion
...
class BAKTPeiyouFusion(nn.Module):
    def __init__(self, n_croutes, n_question, n_pid, 
            d_model, n_blocks, dropout, d_ff=256, 
            num_level = 10, seq_len=200, 
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", dpretrain=dict()):
        super().__init__()
        """
        Input:
        ...
        """
        self.model_name = "bakt_peiyou_fusion"
        ...
        self.q_embed = KCRouteEncoder(num_level, self.n_question, self.n_croutes, emb_type, embed_l, dropout, dpretrain["kc_embs"][0],dpretrain["kc_embs"][1])
        
        self.all_fusion = GateEarlyFusion(embed_l)
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        # Batch First
        kcembs = self.q_embed(kc_routes, q_data)
        q_embed_data = kcembs[0]

        # rasch
        q_embed_diff_data = self.q_embed_diff(q_data) # tailcid
        pid_embed_data = self.difficult_param(pid_data)  # uq 当前problem的难度
        q_embed_data = q_embed_data + pid_embed_data * q_embed_diff_data

        y2, y3 = 0, 0

        qembs = self.question_emb(pid_data, cqtypes) # question infos include qid embedding
        kcothers = kcembs[1:] # kc infos except tailcid
        # x fusion
        infs = [q_embed_data] + qembs + kcothers
        if self.emb_type.find("early") != -1:
            # other side info
            q_embed_data = self.all_fusion(infs)
            # y fusion
            ...
            for inf in infs:
                q_embed_data = inf
                qa_embed_data = self.qa_embed(target) + q_embed_data
                out = self.model(q_embed_data, qa_embed_data)
                outs.append(out)
            d_output = self.all_fusion(outs)
            input_fusion = self.all_fusion(infs)
            concat_q = torch.cat([d_output, input_fusion], dim=-1)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'bakt_peiyou_fusion'}:
