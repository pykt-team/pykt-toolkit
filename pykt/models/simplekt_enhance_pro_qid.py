...
class simpleKT_enhance_pro_qid(nn.Module):
    def __init__(self, n_question, n_pid, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, dataset_name='THINK', embed_size=256):
        super().__init__()
        """
        Input:
        ...
        if self.dataset_name in ["THINK", "jiuzhang_grade3_en"]:
            ...
            raise ValueError(
                "simplekt_enhance_pro_qid only supports datasets with question text embeddings: "
    ...
    def base_emb(self, q_data, target):
        ...
        # 替换成语义embedding
        q_embed_data = self.get_question_embedding(q_data).to(device)
        if self.separate_qa:
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        if self.n_pid > 0 and emb_type.find("norasch") == -1: # have problem id
            if emb_type.find("aktrasch") == -1:  # 启动当前分支
                # q_embed_diff_data = self.q_embed_diff(q_data)  
                # 对问题的 emebedding 进行替换
                q_embed_diff_data = self.get_question_embedding(pid_data).to(device)

                # 对问题的 emebedding 进行替换
                pid_embed_data = self.get_question_embedding(pid_data).to(device)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'simplekt', "simplekt_enhance_pro", "simplekt_enhance_pro_qid"}:
