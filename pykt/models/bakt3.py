...
class BAKT(nn.Module):
    def __init__(self, n_question, n_pid, 
        ...
        if self.emb_type.find("hawkes") != -1:
            ...
            print(f"skill_nums: {self.n_question}")
            print(f"que_nums: {self.n_pid}")
            print(f"emb_size: {embed_l}")
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False, train=False):
        ...
        if emb_type.find("hawkes") != -1:
            inters = q_data + target * self.n_question
            ...
            if emb_type.find("hawkes_only") != -1:
                ...
                preds = m(0.9*output + 0.1*prediction)
        else:
            preds = m(output)
