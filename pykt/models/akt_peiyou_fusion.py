...
import numpy as np
from .peiyou_emb_fusion import KCRouteEncoder, QuestionEncoder, GateEarlyFusion
...
class AKTPeiyouFusion(nn.Module):
    def __init__(self, n_croutes, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if self.n_pid > 0:
            ...
            if emb_type.find("rc") == -1 and emb_type.find("tail") == -1:
                ...
                self.question_fusion = GateEarlyFusion(embed_l)
                self.difficult_param = nn.Sequential(
                    nn.Dropout(self.dropout), nn.ReLU(),
                    nn.Linear(embed_l, 1)
                )
                ####
                self.q_embed_diff = KCRouteEncoder(num_level, self.n_question, self.n_croutes, emb_type, embed_l, dropout, dpretrain["kc_embs"][0],dpretrain["kc_embs"][1])
                self.q_diff_fusion = GateEarlyFusion(embed_l)
            ####
        ...
        if emb_type.find("rc") == -1 and emb_type.find("tail") == -1:
            ...
            self.q_fusion = GateEarlyFusion(embed_l)
            ####
        if self.separate_qa: 
    ...
    def base_emb(self, kc_routes, q_data, target):
        if self.emb_type.find("rc") == -1 and self.emb_type.find("tail") == -1:
            ...
            q_embed_data = self.q_fusion(q_embed_data)
    ...
    def forward(self, dcur, qtest=False):
        ...
        if self.n_pid > 0: # have problem id
            if emb_type.find("rc") == -1 and self.emb_type.find("tail") == -1:
                ...
                q_embed_diff_data  = self.q_diff_fusion(q_embed_diff_data)
                ##
                qembs = self.question_emb(pid_data, cqtypes)
                qembs = self.question_fusion(qembs)
