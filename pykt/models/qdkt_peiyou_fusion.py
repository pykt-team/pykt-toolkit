...
import numpy as np
from .peiyou_emb_fusion import QueBaseModelPeiyou, QuestionEncoder, GateEarlyFusion
from pykt.utils import debug_print

class QDKTNetPeiyouFusion(nn.Module):
    def __init__(self, num_q,num_c,emb_size, dropout=0.1, emb_type='qaid', emb_path=dict(), pretrain_dim=768,device='cpu',mlp_layer_num=1,other_config={}):
        super().__init__()
        self.model_name = "qdkt_peiyou_fusion"
        self.num_q = num_q
        self.num_c = num_c
        ...
        self.fusion = GateEarlyFusion(emb_size)
    ...
    def forward(self, q, qtypes, c ,r,data=None):
        qembs = self.que_emb(q[:,:-1], qtypes[:,:-1])
        fusion_qemb = self.fusion(qembs)

        x = (q + self.num_q * r)[:,:-1]
        xemb = self.interaction_emb(x)

        xemb = xemb + fusion_qemb
...
class QDKTPeiyouFusion(QueBaseModelPeiyou):
    def __init__(self, num_q,num_c, emb_size, dropout=0.1, emb_type='qaid', emb_path={}, pretrain_dim=768,device='cpu',seed=0,mlp_layer_num=1,other_config={},**kwargs):
        ...
        self.model = QDKTNetPeiyouFusion(num_q=num_q,num_c=num_c,emb_size=emb_size,dropout=dropout,emb_type=emb_type,
