...
from .peiyou_emb import QueEmbPeiyou, QueBaseModelPeiyou
from torch.distributions import Categorical
from .iekt_utils import mygru,funcs
from pykt.utils import debug_print

class IEKTNetPeiyou(nn.Module): 
    def __init__(self, num_q,num_croutes,num_c,emb_size,max_concepts,num_level=10,lamb=40,n_layer=1,cog_levels=10,acq_levels=10,dropout=0,gamma=0.93, emb_type='qc_merge', emb_path={}, pretrain_dim=768,device='cpu'):
        super().__init__()
        self.model_name = "iekt_peiyou"
        ...
        self.que_emb = QueEmbPeiyou(num_q=num_q,num_croutes=num_croutes,num_c=num_c,emb_size=emb_size, dropout=dropout,
                             emb_type=self.emb_type, num_level=num_level,
                             emb_paths=emb_path,pretrain_dim=pretrain_dim)

    def get_ques_representation(self, q, qtypes, qcroutes, c):
        ...
        ### TODO
        q = q.unsqueeze(-1)
        qtypes = qtypes.unsqueeze(-1)
        qcroutes = qcroutes.unsqueeze(1)
        c = c.unsqueeze(1)
        # print(f"q: {q.shape}, qcroutes: {qcroutes.shape}, c: {c.shape}")
        # assert False
        v = self.que_emb(q, qtypes, qcroutes, c)
    ...
    def obtain_v(self, q, qtypes, qcroutes, c, h, x, emb):
        """_summary_
        ...
        v = self.get_ques_representation(q, qtypes, qcroutes, c)
...
class IEKTPeiyou(QueBaseModelPeiyou):
    def __init__(self, num_q,num_croutes,num_c,emb_size,max_concepts,lamb=40,n_layer=1,cog_levels=10,acq_levels=10,dropout=0,gamma=0.93, emb_type='qid', emb_path={}, pretrain_dim=768,device='cpu',seed=0):
        model_name = "iekt_peiyou"
        super().__init__(model_name=model_name,emb_type=emb_type,emb_path=emb_path,pretrain_dim=pretrain_dim)

        self.model = IEKTNetPeiyou(num_q=num_q,num_croutes=num_croutes,num_c=num_c,lamb=lamb,emb_size=emb_size,max_concepts=max_concepts,n_layer=n_layer,cog_levels=cog_levels,acq_levels=acq_levels,dropout=dropout,gamma=gamma, emb_type=emb_type, emb_path=emb_path, pretrain_dim=pretrain_dim,device=device)
    ...
    def predict_one_step(self,data,return_details=False,process=True):
        ...
        for seqi in range(0, seq_len):#序列长度
            ...
            # print(f"{data_new['cq'][:,seqi].shape}, {data_new['cqcroutes'][:,seqi].shape}, {data_new['cc'][:,seqi].shape}")
            # assert False
            curqemb = self.model.get_ques_representation(q=data_new['cq'][:,seqi], qtypes=data_new['cqtypes'][:,seqi],
                qcroutes=data_new['cqcroutes'][:,seqi], c=data_new['cc'][:,seqi])
            # print(curqemb.shape, h.shape)
            ques_h = torch.cat([
                curqemb,
                h], dim = 1)#equation4
            # d = 64*3 [题目,知识点,h]
            ...
            h_v, v, logits, rt_x = self.model.obtain_v(q=data_new['cq'][:,seqi], 
                                                        qtypes=data_new['cqtypes'][:,seqi],qcroutes=data_new['cqcroutes'][:,seqi], 
                                                        c=data_new['cc'][:,seqi], 
