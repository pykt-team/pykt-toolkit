...
class timeGap2(nn.Module):
    def __init__(self, num_rgap, num_sgap, num_pcount, emb_size) -> None:
        ...
        if num_sgap != 0:
            self.sgap_eye = torch.eye(num_sgap).to(self.device)
        if num_pcount != 0:
            self.pcount_eye = torch.eye(num_pcount).to(self.device)
        ...
        self.time_emb = nn.Linear(input_size, emb_size, bias=False).to(self.device)

    def forward(self, rgap, sgap, pcount):
        infs = []
        rgap, sgap, pcount = rgap.to(self.device), sgap.to(self.device), pcount.to(self.device)  # Move to the same device as the model
...
class MTKT(nn.Module):
    def __init__(self, n_question, n_pid, num_rgap, num_sgap, num_pcount,k1, k2, 
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    d_model=d_model, d_feature=d_model / num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, k1=k1, k2=k2, seq_len=seq_len)
        ...
        if self.emb_type == "qid":
            ...
            self.model2 = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads,
                                       dropout=dropout, d_model=d_model, d_feature=d_model / num_attn_heads, d_ff=d_ff,
                                       kq_same=self.kq_same, model_type=self.model_type,k1=k1,k2=k2, seq_len=seq_len)
    ...
    def forward(self, dcur, dgaps, qtest=False, train=False):
        ...
        # Batch First
            # print("model2 using CIC, model using FFN")
        if emb_type.startswith("qid"):
        ...
        if emb_type == "qid" or emb_type in ["qidonlyrgap", "qidonlysgap", "qidonlypcount", "qidrsgap", "qidrpgap", "qidspgap"]:
            rg, sg, p = dgaps["rgaps"].long().to(device), dgaps["sgaps"].long().to(device), dgaps["pcounts"].long().to(device)
            rgshft, sgshft, pshft = dgaps["shft_rgaps"].long().to(device), dgaps["shft_sgaps"].long().to(device), dgaps["shft_pcounts"].long().to(device)

            r_gaps = torch.cat((rg[:, 0:1], rgshft), dim=1).to(device)
            s_gaps = torch.cat((sg[:, 0:1], sgshft), dim=1).to(device)
            pcounts = torch.cat((p[:, 0:1], pshft), dim=1).to(device)

            temb = self.time_emb(r_gaps, s_gaps, pcounts)
            # temporal aspect attention
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
                 d_ff, n_heads, dropout, kq_same, model_type, k1,k2,seq_len):
        super().__init__()
        """
        ...
        if model_type in {'mtkt'}:
            self.blocks_2 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same, k1=k1, k2=k2)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 d_ff, n_heads, dropout,  kq_same,k1,k2):
        super().__init__()
        """
        ...
        self.cic = CIC(in_features=d_model, hidden_features=d_ff, kernel_size1=k1, kernel_size2=k2, drop=dropout)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad):
        ...
        scores = attention(q, k, v, self.d_k,
                           mask, self.dropout, zero_pad, self.bias)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, bias):
    """
...
class CIC(nn.Module):
    def __init__(self, in_features, hidden_features, kernel_size1=1, kernel_size2=3, drop=0.1):
        super().__init__()
    ...
    def forward(self, x):
        x = x.transpose(1, 2)  # Transpose to (batch_size, feature_dim, sequence_length)
        x1 = self.conv1(x)
        x1_1 = self.act(x1)
        x1_2 = self.drop(x1_1)
        ...
        x = x.transpose(1, 2)  # Transpose back to (batch_size, sequence_length, feature_dim)
        return x
