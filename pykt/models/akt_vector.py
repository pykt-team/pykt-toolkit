...
class AKTVec(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if self.n_pid > 0:
            ...
            # self.difficult_kc = nn.Embedding(self.n_question+1, embed_l)
        ...
        if emb_type in ["lstmy", "lstmy_bayesian"] and self.use_rasch:
            # n_question+1 ,d_model
            self.q_embed = nn.Embedding(self.n_question + 1, embed_l)
            if self.separate_qa: 
                self.qa_embed = nn.Embedding(2*self.n_question+1, embed_l) # interaction emb
        ...
        if emb_type.startswith("yplus") and self.use_rasch:
            # n_question+1 ,d_model
            self.q_embed = nn.Embedding(self.n_question + 1, embed_l)
            self.que_embed = nn.Embedding(self.n_pid + 1, embed_l)
            if self.separate_qa: 
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        # Batch First
        if emb_type in ["qid", "bayesian", "bernoulli", "bernoulli_v2", "raschy", "relation_bayesian", "relation_bayesian_loss", "lstm", "lstmy", "lstmy_bayesian", "relation_lstmy_bayesian", "bayesian_loss", "yplus_que", "yplus_kc"]:
        ...
        #     # print(f"relation_q_emb: {relation_q_emb.shape}")
        ...
        #     # print(f"relation_q_emb: {relation_q_emb.shape}")
        #     # relation_emb = torch.cat([relation_que_emb, relation_q_emb], dim=2)
        #     # relation_emb = self.linear3(relation_emb)

        if emb_type.startswith("relation"):
            q_embed_data, qa_embed_data = self.base_emb(q_data, target)
            # print(f"self.qmatrix: {self.qmatrix.weight}")
            relation_q = torch.reshape(self.qmatrix(pid_data), [batch_size*seqlen, -1]) # lookup all the kcs
            relation_q_emb = torch.mm(relation_q, self.q_embed.weight)
            concept_num = torch.where(relation_q!= 0, 1, 0).sum(axis=-1).unsqueeze(-1)
            relation_q_emb = torch.reshape(relation_q_emb / concept_num, [batch_size,seqlen,-1])
        ...
        if self.n_pid > 0 and self.use_rasch: # have problem id
            ...
            elif not self.rasch_x and emb_type in ["lstmy", "lstmy_bayesian", "relation_lstmy_bayesian"]:
                qa_embed_data, (h,c) = self.lstm_qa(qa_embed_data)
                qa_embed_diff_data = self.qa_embed_diff(
                    target)  # f_(ct,rt) or #h_rt (qt, rt)差异向量
                if self.separate_qa:
                    qa_embed_data = qa_embed_data + pid_embed_data + \
