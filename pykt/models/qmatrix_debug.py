...
class AKTVec(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if emb_type in ["yplus_ones"] and self.use_rasch:
            ...
            print(f"emb_qmatrix: {self.qmatrix.weight}")
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        # Batch First
        if emb_type in ["qid", "bayesian", "bernoulli", "bernoulli_v2", "raschy", "relation_bayesian", "relation_bayesian_loss", "lstm", "lstmy", "lstmy_bayesian", "relation_lstmy_bayesian", "bayesian_loss", "yplus_ones"]:
        ...
        if self.n_pid > 0 and self.use_rasch: # have problem id
            ...
            if emb_type in ["qid", "bayesian", "bernoulli", "bernoulli_v2", "lstmy", "lstmy_bayesian", "bayesian_loss", "yplus_ones"] :
            ...
            if not self.rasch_x and emb_type in ["qid", "relation", "bernoulli", "bernoulli_v2", "raschy", "bayesian", "relation_bayesian", "relation_bayesian_loss", "lstm", "bayesian_loss"]:
                ...
                if self.separate_qa:
                    ...
                    qa_embed_data = qa_embed_data + pid_embed_data + \
                        (qa_embed_diff_data+q_embed_diff_data)  # + uq *(h_rt+d_ct) # （q-response emb diff + question emb diff）
            elif not self.rasch_x and emb_type in ["yplus_ones"]:
        ...
        if not qtest:
            ...
            print(f"qtest_preds: {preds.shape}")
