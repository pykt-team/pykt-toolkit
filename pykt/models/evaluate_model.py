...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "akt_peiyou", "bakt", "bakt_peiyou", "bakt_simplex", "bakt_time", "bakt_side", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt", "akt_peiyou", "bakt", "bakt_peiyou", "bakt_time", "bakt_side", "bakt_simplex", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def evaluate_splitpred_question(model, data_config, testf, model_name, save_path="", use_pred=False, train_ratio=0.2, atkt_pad=False):
    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            dforget = dict() if model_name not in ["dkt_forget", "bakt_time", "bakt_side"] else get_info_dkt_forget(row, data_config)
