...
class AKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
        ...
        if emb_type.startswith("qid") or emb_type.startswith("forget"):
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_feature, n_heads, dropout, kq_same, bias=True, emb_type="qid"):
        ...
        if emb_type.endswith("avgpool"):
            ...
            self.out_proj = nn.Linear(d_model, d_model, bias=bias)
        elif emb_type.startswith("qid") or emb_type.startswith("forget"):
            self.d_k = d_feature
            self.h = n_heads
            self.kq_same = kq_same
    ...
    def forward(self, q, k, v, mask, zero_pad, pdiff=None):
        ...
        elif self.emb_type.startswith("qid") or self.emb_type.startswith("forget"):
