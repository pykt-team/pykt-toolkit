...
class AKT_Enhance_Pro_qid(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
            kq_same=1, final_fc_dim=512, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, dataset_name='THINK', embed_size=256):
        super().__init__()
        """
        Input:
        ...
        if self.dataset_name in ["THINK", "jiuzhang_grade3_en"]:
            ...
            raise ValueError(
                "akt_enhance_pro_qid only supports datasets with question text embeddings: "
    ...
    def forward(self, q_data, target, pid_data=None, qtest=False):
        ...
        if self.n_pid > 0: # have problem id
            ...
            pid_embed_data = self.get_question_embedding(pid_data)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'akt', "akt_enhance_pro", "akt_enhance_pro_qid"}:
