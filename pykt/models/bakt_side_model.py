...
class BAKTSideModel(nn.Module):
    def __init__(self, n_question, n_pid, num_rgap, num_sgap, num_pcount,  num_prcount, 
            use_rgap, use_sgap, use_pcount, use_prcount, use_pos, gate_scalar,
        ...
        """
        self.model_name = "bakt_side_model"
        ...
        embed_l = d_model
        if self.n_pid > 0:
            self.q_embed_diff = nn.Embedding(self.n_pid+1, embed_l) # 题目难度
        ...
        self.side_infs = [n_question, num_rgap, num_sgap, num_pcount, num_prcount, n_pid, use_pos]
        self.side_models = nn.ModuleList([
            Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads,
            ...
            for inf in self.side_infs if inf > 0
        ])
    ...
    def fusion_fuc(self, fusion_infs):
        ...
        if self.emb_type.endswith("add"):
            ...
            newinfs = []
            for i in range(len(fusion_infs)):
                curinf = fusion_infs[i].unsqueeze(2)
                newinfs.append(self.weights[i](curinf))
            res = torch.cat(newinfs, dim=2)
            after_gate = self.gate(res)
            
            if self.gate_scalar == 1:
                # bz * seqlen * m * h
                weights = torch.sigmoid(after_gate).transpose(-2, -1)
                res = torch.matmul(weights, res).squeeze(-2)

            elif self.gate_scalar == 2:
                weights = torch.softmax(after_gate, dim=-1).transpose(-2, -1)
                res = torch.matmul(weights, res).squeeze(-2)
            else: # == 0
                # gate: matrix
                weights = torch.softmax(after_gate, dim=-2)#.transpose(-2, -1)
                # print(f"res: {res.shape}, weights: {weights.shape}")
                res = torch.sum(weights * res, dim=-2).squeeze(-2)
            # print(f"res: {res.shape}, weights: {weights.shape}")
            # assert False
    ...
    def forward(self, dcur, dgaps, qtest=False, train=False):
        ...
        fusion_infs = [q_embed_data]
        time_infs = self.getside_info(dgaps)
        fusion_infs.extend(time_infs)
        if self.n_pid > 0: # have problem id
            pid_embed_data = self.q_embed_diff(q_data)  # d_ct 总结了包含当前question（concept）的problems（questions）的变化
        ...
        # print(f"fusion_infs: {len(fusion_infs)}, side_models: {len(self.side_models)}")
        assert len(self.side_models) == len(fusion_infs)
        original_input = []#[q_embed_data]
        outs = []
        for i in range(0, len(fusion_infs)):
            curout = self.side_models[i](fusion_infs[i], qa_embed_data)
            outs.append(curout)
            original_input.append(fusion_infs[i])
        # main_out = self.model(q_embed_data, qa_embed_data)
        # outs.append(main_out)
        # print(f"outs: {len(outs)}")
        
        ### fusion
        fusion_out = self.fusion_fuc(outs)
        fusion_input = self.fusion_fuc(original_input) if self.use_pos else self.fusion_fuc(original_input[0:-1])
        # fusion_input = original_input[0]
        # finalend = len(original_input) - 1 if self.use_pos else len(original_input)
        # for i in range(1, finalend):
        #     fusion_input = fusion_input + original_input[i]

        ###
        # print(f"fusion_out: {fusion_out.shape}, ")
        # print(f"fusion_input: {fusion_input.shape}, ")
        concat_q = torch.cat([fusion_out, fusion_input], dim=-1)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
        ...
        if model_type in {'bakt_side_model'}:
    ...
    def forward(self, q_embed_data, qa_embed_data):
        ...
        # q_posemb = self.position_emb(q_embed_data).to(device)
        # # print(f"q_embed_data device: {q_embed_data.device}, q_posemb: {q_posemb.device}")
        # q_embed_data = q_embed_data + q_posemb
        # qa_posemb = self.position_emb(qa_embed_data).to(device)
