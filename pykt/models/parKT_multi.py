...
class parKT(nn.Module):
    ...
    def __init__(self, n_question, n_pid, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid", emb_path="", pretrain_dim=768, augment_type="random", tao=0.2, gamma=0.7, beta=0.2, n_views=2,cf_weight=0.1, seq_representation_instancecl_type="mean",temperature=1.0, num_stu=None):
        super().__init__()
        """
        Input:
        ...
        if emb_type.find("ele") != -1:
            # self.stu_embed = nn.Embedding(self.num_stu, embed_l)
            self.lstm_layer = LSTM(embed_l*2, embed_l, batch_first=True)
        else:
            if emb_type.find("rnn") == -1:
    ...
    def forward(self, dcur, qtest=False, train=False):
        ...
        # if emb_type.find("uid") != -1:
        #     uid_data = dcur["uid"]
        #     stu_embed_data = self.stu_embed(uid_data).repeat(1,q_data.size(1),1)
            # print(f"stu_embed_data:{stu_embed_data.shape}")
        ...
        if emb_type.startswith("qid"):
            ...
            # dim_size = q_embed_data.size(2)
            # nums = torch.arange(seqlen+1)[1:].unsqueeze(-1).to(device)
            # hidden = torch.cumsum(q_embed_data,dim=1)/nums
            # pad_zero = torch.zeros(bs, 1, dim_size).to(device)
            # hidden = torch.cat([pad_zero, hidden[:bs, 0:-1, :]], dim=1) # 第一行score置0
            if emb_type.find("ele") != -1:
                query = q_embed_data + hidden
                d_output = self.model(query, qa_embed_data)
                input_combined = torch.cat((d_output, query), -1)
                rnn_out,_ = self.lstm_layer(input_combined)
                # print(f"rnn_out:{rnn_out.shape}")
                output = self.out(rnn_out).squeeze(-1)  
            elif emb_type.find("rnn") != -1:
                # print(f"q_embed_data:{q_embed_data.shape}")
                query, _ = self.emb_lstm(q_embed_data)
                # key, _ = self.y_emb_lstm(qa_embed_data)   
                # print(f"query:{query.shape}")
                d_output = self.model(query, qa_embed_data)   
                # d_output = self.model(query, key)   
                input_combined = torch.cat((d_output, query), -1)
                # rnn_out,_ = self.lstm_layer(input_combined)
                # print(f"rnn_out:{rnn_out.shape}")
                # output = self.out(rnn_out).squeeze(-1)     
                ...
                if emb_type.find("pvn") != -1 and train:
                    neg_cids = []
                    total_c = torch.arange(0, self.n_question)
                    for i in range(q_data.size(0)):
                        pos_sample = q_data[i].detach().cpu().numpy()
                        neg_list = list(set(np.array(total_c)).difference(set(pos_sample)))
                        neg_item = choice(neg_list)
                        neg_cids.append(neg_item)
                    neg_cids = torch.tensor(neg_cids, dtype=torch.long, device=q_data.device).repeat(q_data.size(1),1).permute(1,0)
                    neg_q_embed_data, neg_qa_embed_data = self.base_emb(neg_cids, target)
                    
                    if self.n_pid > 0 and emb_type.find("norasch") == -1: # have problem id
                        neg_q_embed_diff_data = self.q_embed_diff(neg_cids)  # d_ct 总结了包含当前question（concept）的problems（questions）的变化
                        neg_pid_embed_data = self.difficult_param(pid_data)  # uq 当前problem的难度
                        neg_q_embed_data = neg_q_embed_data + neg_pid_embed_data * \
                            neg_q_embed_diff_data  # uq *d_ct + c_ct # question encoder

                        neg_qa_embed_diff_data = self.qa_embed_diff(
                            target)  # f_(ct,rt) or #h_rt (qt, rt)差异向量
                        neg_qa_embed_data = neg_qa_embed_data + neg_pid_embed_data * \
                                (neg_qa_embed_diff_data+neg_q_embed_diff_data)  # + uq *(h_rt+d_ct) # （q-response emb diff + question emb diff）
                        neg_d_output = self.model(neg_q_embed_data, neg_qa_embed_data)
                        neg_input_combined = torch.cat((neg_d_output, neg_q_embed_data), -1)
                        # hidden = F.relu(self.i2h(input_combined))
                        # self.hidden = hidden
                        neg_rnn_out, _ = self.lstm_layer(neg_input_combined)
                        neg_output = self.out(neg_rnn_out).squeeze(-1)
                        # print(f"neg_output:{neg_output.shape}")
                        istarget = (cshft >= 0).view(cshft.size(0), cshft.size(1)).float() # [batch*seq_len]
                        # print(f"istarget:{istarget}")
                        # print(f"pos_logits:{pos_logits[:,1:].squeeze(-1).shape}")
                        cl_losses = self.cf_weight * torch.sum(torch.clamp(output[:,1:] - neg_output[:,1:], 0) * istarget) / torch.sum(istarget)

            m = nn.Sigmoid()
            preds = m(output)
        if emb_type == "qid_mt" and train:
            cl_losses = []
            sm = dcur["smasks"].long()
            start = 0
            # print(f"rnn_out:{rnn_out.shape}")
            next_c_pred = self.out_2(rnn_out)
        ...
        elif emb_type.find("cl") != -1 and train:
            cl_losses = []
            ...
            for idx,cl_batch in enumerate(cl_batches):
                ...
                hidden = torch.cumsum(q_embed_data,dim=1)/nums
                pad_zero = torch.zeros(q_embed_data.size(0), 1, q_embed_data.size(2)).to(device)
                hidden = torch.cat([pad_zero, hidden[:q_embed_data.size(0), 0:-1, :]], dim=1) # 第一行score置0
                if self.n_pid > 0:
                ...
                query = torch.cat((q_embed_data, hidden), 2)
                query = self.linear(query)
                cl_loss = self._instance_cl_one_pair_contrastive_learning(cl_batch, query, qa_embed_data)
                cl_losses.append(self.cf_weight * cl_loss)            

        if train:
            if emb_type not in ["qid_cl", "qid_mt", "qid_pvn"]:
                return preds, y2, y3
            else:
