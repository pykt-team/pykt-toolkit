...
def get_outputs(self,emb_qc_shift,h,data,add_name="",model_type='question'):
    ...
    if model_type == 'question':
        ...
        outputs["y_concept_all_raw"+add_name] = y_concept_all
...
class xDKTV2Net(nn.Module):
    ...
    def forward(self, q, c ,r,data=None):
        ...
        outputs.update(concept_outputs)#.update(outputs)
...
class xDKTV2(QueBaseModel):
    ...
    def predict(self,dataset,batch_size,return_ts=False,process=True):
        ...
        with torch.no_grad():
            ...
            for data in test_loader:
                ...
                for key in outputs:
                    if not key.startswith("y") or key in ['y_qc_predict'] or 'raw' in key:
                        continue
    ...
    def evaluate(self,dataset,batch_size,acc_threshold=0.5,return_raw=False):
        ...
        if return_raw:
            eval_result['raw_result'] = {"y_pred":results['y'],"y_true":ts}
