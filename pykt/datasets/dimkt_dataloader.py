...
class DIMKTDataset(Dataset):
    def __init__(self,file_path,input_type,folds,qtest=False):
        ...
        if not os.path.exists(processed_data):
            ...
            if self.qtest:
                self.dori, self.dqtest = self.__load_data__(self.sequence_path, folds)
                save_data = [self.dori, self.dqtest]
            else:
                self.dori = self.__load_data__(self.sequence_path, folds)
                save_data = self.dori
    ...
    def __load_data__(self,sequence_path,folds,pad_val=-1):
        ...
        for i,row in df.iterrows():
            ...
            if "skill_difficult" in row:
                dori["sdseqs"].append([int(_) for _ in row["skill_difficult"].split(",")])
            if "question_difficult" in row:
                dori["qdseqs"].append([int(_) for _ in row["question_difficult"].split(",")])
