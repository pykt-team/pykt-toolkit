...
def init_test_datasets(data_config, model_name, batch_size):
    ...
    if model_name in ["dkt_forget", "bakt_time", "bakt_side",  "bakt_side_model"]:
    ...
    elif model_name in ["akt_peiyou",  "akt_peiyou_fusion", "bakt_peiyou",  "bakt_peiyou_fusion"]:
        ...
        test_question_dataset = None
        test_question_window_dataset= None
    elif model_name in que_type_models and model_name not in ["iekt_peiyou", "qdkt_peiyou",  "qdkt_peiyou_fusion"]:
    ...
    if model_name in ["akt_peiyou",  "akt_peiyou_fusion", "bakt_peiyou",  "bakt_peiyou_fusion"]:
...
def update_gap(max_rgap, max_sgap, max_pcount, max_prcount, cur):
    ...
    max_prcount = cur.max_prcount if cur.max_prcount > max_prcount else max_prcount
    return max_rgap, max_sgap, max_pcount, max_prcount
...
def init_dataset4train(dataset_name, model_name, data_config, i, batch_size):
    ...
    if model_name in ["dkt_forget", "bakt_time", "bakt_side",  "bakt_side_model"]:
        ...
        max_rgap, max_sgap, max_pcount, max_prcount = update_gap(max_rgap, max_sgap, max_pcount, max_prcount, curtrain)
        max_rgap, max_sgap, max_pcount, max_prcount = update_gap(max_rgap, max_sgap, max_pcount, max_prcount, curvalid)
    ...
    try:
        if model_name in ["dkt_forget", "bakt_time", "bakt_side",  "bakt_side_model"]:
            ...
            # test_window_dataset = DktForgetDataset(os.path.join(data_config["dpath"], data_config["test_window_file"]),
            #                                 data_config["input_type"], {-1})
            max_rgap, max_sgap, max_pcount, max_prcount = update_gap(max_rgap, max_sgap, max_pcount, max_prcount, test_dataset)
    except:
        pass
    ...
    if model_name in ["dkt_forget", "bakt_time", "bakt_side",  "bakt_side_model"]:
        ...
        data_config["num_prcount"] = max_prcount + 1
    if model_name == "lpkt":
