...
def main(params):
    ...
    model.train(train_dataset, valid_dataset, batch_size=16,
                num_epochs=params['num_epochs'], patient=5, shuffle=False, save_model=True, save_dir=save_dir)
    model.load_model(model.save_dir)

    auc,acc = model.evaluate(test_dataset,batch_size=16)
    auc_win,acc_win = model.evaluate(test_win_dataset,batch_size=16)
